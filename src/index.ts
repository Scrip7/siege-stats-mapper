...
app.get('/', (req, res) => {
	...
	.then(response => {
		const regexResults = /<script>window\.__PRELOADED_STATE__ \= (.*?)<\/script>/gs.exec(response.data);
		if (_.isNull(regexResults)) throw new Error('Could not find any preload data');
...
function mapLoadout(loadouts: any) {
	...
	loadouts.map((loadout: any) => {
		...
		if (_.isUndefined(response[loadout.weaponType])) response[loadout.weaponType] = [];
...
function mapWeapons(weapons: any) {
    ...
    weapons.map((weapon: any) => {
        ...
        if (_.isUndefined(response[weapon.weaponType])) response[weapon.weaponType] = [];
        response[weapon.weaponType].push({
            slug: weapon.slug,
            title: weapon.localizedItems ? weapon.localizedItems.title : undefined,
            categoryName: weapon.localizedItems ? weapon.localizedItems.weaponSubtype : undefined,
